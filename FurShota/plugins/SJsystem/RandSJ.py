...
CM_got_sj_by_id = on_command("看看",to_me,aliases={"查看","查看圣经"},priority=7,block = True)
...
async def func_rudian(bot: Bot,event: Event):
    ...
    if event.reply:
        ...
        print("入典")
        sj_db.insert_SJ(referenced_sender_id,referenced_message_id)
...
async def func_got_sj_by_id(bot: Bot,event: Event):
    ...
    if at_ids:
        ...
        print(readytosend)
        await bot.call_api("send_group_forward_msg",messages=GenerateSJ(readytosend),group_id=test_group_id)      
    else:
        print("没有 @ 的对象")
